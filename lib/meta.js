...
Meta.prototype = {
  ...
  digit: function () {
    ...
    this.index--;
    throw new ParseError(this.index + ': expected digit, got: "' + chr + '"');
  },
  ...
  array: function() {
    ...
    this.index--;
    throw new ParseError(this.index + ': expected array, got: "' + any + '"');
  },
...
Meta._apply = function (name, rule){
  ...
  if(!record){
    ...
    record = input.save(name, args, leftRecursion, this.index);
    try {
      var result = rule.apply(this, args);
      record = input.save(name, args, result, this.index);
      
      if(leftRecursion.detected){
        var sentinel = this.index;
        while(true){
          try {
            this.index = index;
            result = rule.apply(this, args);
            if(this.index == sentinel){
              break;
            }
            record = input.save(name, args, result, this.index);
          } catch(e) {
